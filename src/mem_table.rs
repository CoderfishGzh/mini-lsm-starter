...
impl MemTable {
    ...
    pub fn create_with_wal(id: usize, _path: impl AsRef<Path>) -> Result<Self> {
        Ok(Self {
            map: Arc::new(SkipMap::new()),
            wal: None,
            id,
    ...
    pub fn put(&self, key: &[u8], value: &[u8]) -> Result<()> {
        ...
        self.approximate_size
            .fetch_add(insert_size, std::sync::atomic::Ordering::Relaxed);
        Ok(())
    }
    ...
    pub fn scan(&self, lower: Bound<&[u8]>, upper: Bound<&[u8]>) -> MemTableIterator {
        ...
        let entry = iter.with_iter_mut(|iter| iter.next());
...
impl StorageIterator for MemTableIterator {
    ...
    fn next(&mut self) -> Result<()> {
        let item = self.with_iter_mut(|iter| iter.next());
        let item = item
            .map(|e| (e.key().clone(), e.value().clone()))
            .unwrap_or((Bytes::new(), Bytes::new()));
        self.with_mut(|iter| {
